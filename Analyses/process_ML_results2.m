...
function process_ML_results2(ind, finalparameters)
    ...
    for cell_index=1:length(ind)
        ...
        if exist(sprintf('%s/glm_cell_%i_v_%i.mat',params.folder_mat,cell_no,params.plot_version), 'file')==2 
            ...
            %if (abs(peak) > .001)
                %saveas(gcf,sprintf('ecog-eeg/stim_specific_time_cells/Buffalo/finalcell_%i_v_%i.jpeg',cell_no,params.plot_version));
                saveas(gcf,sprintf('ecog-eeg/stim_specific_time_cells/Buffalo/corrcells_%i_v_%i.jpeg',cell_no,params.plot_version));
